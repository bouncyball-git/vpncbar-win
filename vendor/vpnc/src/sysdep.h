...
/* native Windows (VpncBar port): winsock + shims instead of the BSD headers */
#include "sysdep-win32.h"
#else
#include <sys/types.h>
...
/***************************************************************************/
#if defined(_WIN32) && !defined(__CYGWIN__)
/* mingw-w64 provides asprintf/vasprintf with _GNU_SOURCE; setenv/unsetenv
 * and error() are implemented in sysdep-win.c */
#define HAVE_VASPRINTF 1
#define HAVE_ASPRINTF 1
#endif
...
/***************************************************************************/
#if defined(__CYGWIN__) || (defined(_WIN32) && !defined(__CYGWIN__))
/*
 ...
 * (shared with the native Windows port — only referenced by tap mode,
 * which is compiled but rejected at runtime there)
 */
