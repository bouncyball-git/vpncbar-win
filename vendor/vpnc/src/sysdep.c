...
/* xnu 1456.1.26 (OS X 10.6)+ provides native user tunnels (utun), which are
 * PF_SYSTEM sockets rather than character devices and need no tuntaposx kext. */
#include <ctype.h>
#include <sys/ioctl.h>
...
/* Native utun support (no tuntaposx kext). Ported from breiter/vpnc 0.5.3-xnu.
 *
 * We probe utun units explicitly instead of letting the kernel pick (sc_unit=0)
 * because auto-allocation is unreliable across sleep/resume on some Darwin
 * versions (it can reuse a dead utun or allocate one that panics on access). */
#define XNU_NO_UTUN_IFACE_NAME -3
#define XNU_UTUN_UNSUPPORTED   -2
#define XNU_UTUN_IN_USE        -1

/* Open one specific utun unit. <0 return codes per the macros above. */
static int xnu_open_utun(int utunnum)
{
	...
	if (fd < 0) {
		printf("Opening utun (%s): %s\n", "socket(SYSPROTO_CONTROL)", strerror(errno));
		return XNU_UTUN_UNSUPPORTED;
	}
	if (ioctl(fd, CTLIOCGINFO, &ctlInfo) == -1) {
		close(fd);
		printf("Opening utun (%s): %s\n", "ioctl(CTLIOCGINFO)", strerror(errno));
		return XNU_UTUN_UNSUPPORTED;
	}
	...
	sc.sc_unit    = utunnum + 1; /* utunX where X == sc_unit - 1 */

	if (connect(fd, (struct sockaddr *)&sc, sizeof(sc)) < 0) {
		close(fd); /* unit already in use */
		return XNU_UTUN_IN_USE;
	}
	return fd;
}

/* Find and open the first free utun unit; stores its name (e.g. "utun4") in dev. */
int utun_open(char *dev)
{
	int fd = XNU_UTUN_UNSUPPORTED;
	...
	for (utunnum = 0; utunnum < 255; utunnum++) {
		...
		if (fd != XNU_UTUN_IN_USE)
			break; /* got one, or hit an unrecoverable error */
	}
...
#if defined(__APPLE__)
	/* Prefer kext-free native utun; fall back to /dev/tunN (tuntaposx) below. */
	if (mode == IF_MODE_TUN) {
		fd = utun_open(dev);
		if (fd >= 0)
			return fd;
...
#if defined(__APPLE__)
/* utun prepends a 4-byte address-family header to every packet. Strip it from
 * the byte count we report so the rest of vpnc sees plain IP packet lengths. */
static inline ssize_t utun_strip_af_header(ssize_t len)
{
	if (len > 0)
		return len > (ssize_t)sizeof(uint32_t) ? len - sizeof(uint32_t) : 0;
	return len;
}
...
ssize_t utun_read(int fd, uint8_t *buf, int len)
{
	uint32_t type;
	struct iovec iv[2];
	iv[0].iov_base = &type;            /* 4-byte AF header lands here, discarded */
	iv[0].iov_len  = sizeof(type);
	iv[1].iov_base = buf;
	iv[1].iov_len  = len;
	return utun_strip_af_header(readv(fd, iv, 2));
}
...
ssize_t utun_write(int fd, uint8_t *buf, int len)
{
	...
	type = htonl(iph->ip_v == 6 ? AF_INET6 : AF_INET);
	iv[0].iov_base = &type;
	iv[0].iov_len  = sizeof(type);
	iv[1].iov_base = buf;
	iv[1].iov_len  = len;
	return utun_strip_af_header(writev(fd, iv, 2));
}
#endif /* __APPLE__ */
