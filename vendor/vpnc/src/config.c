...
#if defined(_WIN32) && !defined(__CYGWIN__)
/* The Password-helper directive spawns an external program via fork/exec/pipe;
 * VpncBar never sets it (secrets arrive on stdin), so it's a no-op on Windows. */
...
char *vpnc_getpass(const char *prompt)
{
	...
	/* VpncBar always feeds the full config (incl. secrets) on stdin, so this
	 * interactive path never runs; on Windows we just read a line, no echo
	 * toggling (no termios). */
