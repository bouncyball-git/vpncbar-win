/* IPSec ESP and AH support.
   ...
   SPDX-FileCopyrightText: 1999      Pierre Beyssac
   SPDX-FileCopyrightText: 2002      Geoffrey Keating
   SPDX-FileCopyrightText: 2003-2007 Maurice Massar
...
/* get_current_dir_name() is a GNU/glibc extension; emulate it on macOS/BSD.
 * getcwd(NULL, 0) malloc()s a buffer here, matching the free(cwd) caller. */
...
/* On macOS the tunnel may be a native utun (name "utunN"), whose packets carry
 * a 4-byte AF header handled by utun_read/utun_write; otherwise it's a classic
 * /dev/tunN char device. Dispatch by interface name. Other platforms are plain. */
#if defined(__APPLE__)
#define TUN_WRITE(s, buf, len)                                          \
	(strncmp((s)->tun_name, "utun", 4) == 0                         \
		 ? utun_write((s)->tun_fd, (uint8_t *)(buf), (len))     \
		 : tun_write((s)->tun_fd, (unsigned char *)(buf), (len)))
#define TUN_READ(s, buf, len)                                           \
	(strncmp((s)->tun_name, "utun", 4) == 0                         \
		 ? utun_read((s)->tun_fd, (uint8_t *)(buf), (len))      \
		 : tun_read((s)->tun_fd, (unsigned char *)(buf), (len)))
#else
#define TUN_WRITE(s, buf, len) tun_write((s)->tun_fd, (buf), (len))
#define TUN_READ(s, buf, len) tun_read((s)->tun_fd, (buf), (len))
#endif
...
/* On Windows the tunnel device isn't a selectable fd, so a dedicated
 * thread pumps it (the strategy the Cygwin port established): blocking
 * tun_read()s, encapsulate, send. The socket side stays in select(). */
...
static DWORD WINAPI tun_thread_win32(LPVOID arg)
{
	tun_thread(arg);
	return 0;
}
...
#if defined(_WIN32) && !defined(__CYGWIN__)
	/* loopback self-pipe so a stop request interrupts select() */
	wake_fd = win32_wake_socket();
	FD_SET(wake_fd, &rfds);
	nfds = MAX(nfds, (int)wake_fd + 1);
#endif
...
#elif defined(_WIN32)
	if (CreateThread(NULL, 0, tun_thread_win32, s, 0, NULL) == NULL) {
		logmsg(LOG_ERR, "Cannot create tun thread!\n");
		return;
	}
#endif
...
#if defined(_WIN32) && !defined(__CYGWIN__)
		if (FD_ISSET(wake_fd, &refds)) {
			char drain[8];
			recv(wake_fd, drain, sizeof(drain), 0);
			continue;   /* do_kill is checked by the loop condition */
		}
#endif
...
#if defined(_WIN32) && !defined(__CYGWIN__)
	/* No signals on Windows: the VpncBar service signals a named event
	 * (VPNCBAR_STOP_EVENT) for graceful disconnect; Ctrl+C covers dev use. */
	win32_start_stop_watcher();
#else
...
#if defined(_WIN32) && !defined(__CYGWIN__)
	/* No fork() on Windows — the service keeps vpnc as a foreground
	 * child (which is also what it wants for log capture + lifetime). */
